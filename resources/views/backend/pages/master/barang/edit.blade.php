...
<div class="d-sm-flex align-items-center justify-content-start mb-4">
    ...
    <h1 class="h3 mb-0 text-gray-800">Edit Barang <b>{{ $item->name }}</b></h1>
</div>
...
<div class="card shadow">
	<div class="card-body">
			...
			@endif
		<form action="{{ route('master.barang.update', $item->id) }}" method="post" enctype="multipart/form-data">
		@csrf
		@method('PUT')
		...
		<div class="form-group">
			...
			<select name="id_kategori" id=""
				...
				@foreach ($kategori_list as $kat)
					...
					@if(old('id_kategori'))
						{{ 'selected' }}
					@elseif($item->id_kategori == $kat->id)
						{{ 'selected' }}
					@endif>
				{{ $kat->kategori_barang }}
				</option>
		...
		<div class="form-group">
			...
			<select name="unit_satuan" id=""
				...
				<option value="Unit" @if (old('unit_satuan') ?? $item->unit_satuan == 'Unit') {{ 'selected' }} @endif>Unit</option>
				<option value="Pcs" @if (old('unit_satuan') ?? $item->unit_satuan == 'Pcs') {{ 'selected' }} @endif>Pcs</option>
				<option value="Pack" @if (old('unit_satuan') ?? $item->unit_satuan == 'Pack') {{ 'selected' }} @endif>Pack</option>
			</select>
			@error('unit_satuan')
			<div class="invalid-feedback">{{$message}}</div>
			@enderror
